/**
 * Taldea Detail Manager - Taldeen Xehetasunen Kudeaketa Sistema
 * 
 * Sistema osoa talde baten xehetasunak bistaratzeko.
 * Funtzionalitate nagusiak:
 * - URL-tik taldearen izena ateratzea
 * - XML fitxategitik taldearen datuak kargatzea
 * - Taldearen informazioa bistaratzea
 * - Kanpoko estekak kudeatzea
 * - Erroreen kudeaketa
 * 
 * Eragiketa logika:
 * 1. URL parametroak irakurtzen ditu (URLSearchParams)
 * 2. Taldearen izena balidatzen du
 * 3. Taldeak.xml fitxategia kargatzen du
 * 4. XPath erabiliz taldea bilatzen du
 * 5. Datuak HTML-n bistaratzen ditu
 * 6. Erroreen kasuan mezua erakusten du
 * 
 * @author EEF
...
/**
 * Bide-oinarria ebazteko funtzio laguntzailea
 * 
 * Funtzio honek bide erlatiboak bide absolutuak bihurtzen ditu:
 * - Irudiak eta beste baliabideentzat erabiltzen da
 * - Absolute URL-ak eta /-rekin hasten direnak ez aldatzen
 * - Beste kasuetan base bidea gehitzen du
 * 
 * @param {string} maybeRelativePath - Bide erlatiboa edo absolutua
 * @return {string} Bide osoa
 */
function eefResolveAssetPath(maybeRelativePath) {
    if (!maybeRelativePath) return "";
    const p = String(maybeRelativePath).trim();
    if (!p) return "";
    // Absolute edo URL bada, zuzenean itzuli
